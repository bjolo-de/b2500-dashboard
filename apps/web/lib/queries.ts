...
// Server-side daily rollup for week/month views: one HTTP call returns
// ~7–31 pre-aggregated rows instead of fanning out tens of paginated reads
// to dump tens of thousands of raw samples.
//
// Reads go through the `daily_aggregates_cached` RPC (migration 0004), which
// serves completed days from the daily_rollups table and recomputes only the
// in-progress day. This sidesteps the ~3 s statement_timeout on the anon
// role: scanning raw rows live took 2.6 s for a week and timed out (HTTP 500)
// for anything past ~8 days, so the previous chunking approach never worked.
export async function fetchDailyAggregates(
  from: Date,
  to: Date,
  tz: string = "UTC",
): Promise<DailyAggregateRow[]> {
  const { data, error } = await supabase.rpc("daily_aggregates_cached", {
